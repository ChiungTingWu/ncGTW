...
CPWmin = 2
# UPLC peakwidths can be as narrow as this - in practice, they are not usually
# so short but the other parameters ensure noise peaks are not prevalent
CPWmax = 25
# widest peak appears to be about 20s but I generally add some time to this as
# peaks are not easy to deconvolve by eye
ppm = 15
# calculated from the raw data - see spreadsheet in zipfile
xsnthresh = 3
# generous, but minimum acceptable S/N
#snth = 10
LM= FALSE
# in practice, including LM files does not appear to really influence final
# result - also peakpicking does not always appear to work when included
integrate=2
# raw data of sufficient quality that it can be used directly. Would consider
# waves in presence of strong peak asymmetry/skew, but generally unecessary
RTerror=6
# check drift of peak tops from chromatograms - see spreadsheet in zipfile
MZerror=0.05
prefilter=c(8, 1000)
mzdiff = -0.001
...
tempFiles <- list.files(temp_path, pattern = "mzXML",
...
for (n in 1:length(tempFiles))
{
    ...
    tempInd[n] <- as.numeric(substr(tempCha, regexpr("_iQC", tempCha) + 4,
                                    tempLen - 6))
}
...
bw = 7 # adjust
Sys.time()
xcmsLargeWin <- group(agds, method="density", mzwid=MZerror,
                      bw=bw, minfrac = 0.5)
Sys.time()
xcmsSmallWin <- group(agds, method="density", mzwid=MZerror,
                      bw=0.3, minfrac = 0.00001, minsamp = 2)
Sys.time()
...
for (n in 1:dim(excluGroups)[1]){
    ncGTWoutputs[[n]] <- ncGTW(ncGTWinputs[[n]], xcmsLargeWin, 50,
                               bpParam = SnowParam(workers = 8))
    allTime[[n]] <- Sys.time()
    cat(format(Sys.time()), 'Group', ncGTWinputs[[n]]$groupInfo['index'],
        'is realigned.\n')
}
...
savePath <- "C:/Users/cbil/Google Drive/5th project/XCMS_analysis/
mesa/iQC335_040419/figure/ncgtw"
