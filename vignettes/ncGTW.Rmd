...
To detect misaligned features, `misalignDetect()` needs two different XCMS
...
There are two features are detected as shown in `excluGroups`. Before realigning
them, the raw profile of each detected feature of each sample needs to load from
the files. `loadProfile()` loads the needed information with file paths (`file`)
and the detected features (`excluGroups`) as inputs.
...
from 0.369 to 0.238, and for the second one, the CV decreases from 0.351 to
0.128. Thus, it is very clear that new warping functions improve the quality of
